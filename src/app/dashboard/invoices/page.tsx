...
import { FaPlus, FaDownload, FaEye, FaTrash, FaFilter, FaSort, FaSpinner, FaFileInvoiceDollar } from 'react-icons/fa';
...
export default function InvoicesPage() {
  ...
  const [sortBy, setSortBy] = useState<SortableInvoiceField>('created_at');
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Invoice ID</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedInvoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900">#{invoice.id.slice(0, 8)}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900">{invoice.client_id}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900">{invoice.total} {invoice.currency}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900">{new Date(invoice.created_at).toLocaleDateString()}</div>
                    </td>
                    <td className="px-6 py-4">
        ...
        {sortedInvoices.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="mx-auto h-12 w-12 bg-gray-100 rounded-full flex items-center justify-center mb-4">
              <FaFileInvoiceDollar className="h-6 w-6 text-gray-400" />
            </div>
